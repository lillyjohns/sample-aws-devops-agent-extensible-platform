...
5. **Registration validation means create-order matters:** the runtime must be READY and serving its card before RegisterService succeeds — a failed registration rolls back the whole stack including a just-updated runtime artifact. The stack supports `-c registerRemoteAgent=false` for two-phase bring-up.
