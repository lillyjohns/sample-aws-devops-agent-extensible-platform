...
## A2A deployment findings (verified in ap-northeast-1, 2026-07-22)

From bringing up the remediation A2A agent (M3) — `GovernanceBlueprint-RemediationAgent`:

1. **CFN `AWS::DevOpsAgent::Service`/`::Association` don't cover `remoteagent`/`remoteagentsigv4` yet** (registry schema ServiceType enum stops at the MCP/SaaS types), while the control-plane API does (`RegisterService` supported enum includes both). The blueprint bridges with a small custom resource (`platform/custom-resources/remote-agent-registration/`) that drives RegisterService/AssociateService/Disassociate/Deregister — the lifecycle stays in CloudFormation, and the construct swaps to pure CFN the day the types catch up.
2. **`AWS::BedrockAgentCore::Runtime` with `ProtocolConfiguration: A2A` + `CodeConfiguration` works in Tokyo** — a zip asset + `PYTHON_3_13` + `entryPoint: [agent.py]`, no container build. Two gotchas: the runtime create **rejects zips containing `__pycache__`** ("Python cache files that are incompatible with the target runtime" — exclude them from the asset), and the managed Python runtime **does not bundle boto3** — the agent signs its one `lambda:Invoke` call with a ~100-line stdlib SigV4 signer using the container credential endpoint (`AWS_CONTAINER_CREDENTIALS_FULL_URI`).
3. **`RegisterService(remoteagentsigv4)` validates the endpoint at registration time by GETting it as-is** through the provided invoke role (assumed-role session name `sigv4McpServerSession`). Two consequences: the registered endpoint must be the **agent-card URL** (`.../runtimes/<url-encoded-arn>/invocations/.well-known/agent-card.json` — registering the bare `/invocations/` URL fails with the 404/405 the data plane returns for GET), and the invoke role needs **`bedrock-agentcore:GetAgentCard`** in addition to `InvokeAgentRuntime` (without it registration fails with an explicit AccessDenied naming that action). Same aidevops.amazonaws.com trust + confused-deputy conditions as the Gateway invoke role.
4. **The association is trivially thin:** `configuration.remoteagentsigv4` is an empty structure — no per-association tool allowlist like MCP. The governance boundary for an A2A agent is the agent's own IAM (here: the runtime role can invoke exactly one Lambda) plus what its card advertises, which is why the manifest's `scope` clause matters.
5. **CodeConfiguration runtimes get NO credential env vars** — no `AWS_ACCESS_KEY_ID`, no `AWS_CONTAINER_CREDENTIALS_FULL_URI` (observed env: only `AWS_REGION`/`AWS_DEFAULT_REGION`/`AWS_EXECUTION_ENV`). The runtime role's credentials are served via **IMDSv2** (`169.254.169.254`, token → `/latest/meta-data/iam/security-credentials/<role>`); the agent's stdlib signer resolves container-endpoint → env → IMDS in that order. boto3-based agents get this for free from the default chain — hand-rolled signers must implement it.
6. **Chat executions don't surface a delegation tool for `remoteagentsigv4` associations yet** (as of 2026-07-22): the orchestrator sees the association via `list_associations` but `search_user_tools` returns no remote-agent invocation tool — protocol-level A2A works (see docs/a2a-evidence.md §1), and the registration is validated at create time, but NL delegation from chat isn't wired by the service yet. Keep the registration in IaC; delegation lights up service-side.
7. **Registration validation means create-order matters:** the runtime must be READY and serving its card before RegisterService succeeds — a failed registration rolls back the whole stack including a just-updated runtime artifact. The stack supports `-c registerRemoteAgent=false` for two-phase bring-up.

## Open items

- [x] Check APIs for A2A registration → wrapped as a CFN custom resource (M3: `Custom::DevOpsAgentRemoteAgent`)
...
- [ ] Check APIs for webhook/HMAC registration → wrap as CFN custom resources if possible (June 2026 releases added API-key webhook auth and Asset APIs — likely yes)
